    ...
    <section id="dip-radar" className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-12">
      <div className="text-center max-w-3xl mx-auto space-y-3">
        ...
        <h2 className="text-3xl sm:text-5xl font-black font-syne tracking-tight text-brand-dark">
          The 7 Proprietary House Emulsions
        </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
        ...
        <div className="lg:col-span-6 relative flex flex-col justify-center items-center bg-brand-dark p-8 sm:p-10 rounded-3xl border border-white/10 shadow-2xl overflow-hidden">
          <div className="relative w-72 h-72 sm:w-80 sm:h-80 flex items-center justify-center">
            <svg viewBox="0 0 300 300" className="w-full h-full">
              ...
              <polygon
                ...
                stroke="#E23A0B"
                strokeWidth="2.5"
                className="transition-all duration-500 ease-out filter drop-shadow-[0_0_12px_rgba(226,58,11,0.5)]"
              />
            </svg>

            {/* Radar Metric Axis Labels */}
            <span className="absolute top-1 text-[11px] font-mono font-bold text-brand-glaze">UMAMI</span>
            <span className="absolute right-1 top-1/3 text-[11px] font-mono font-bold text-brand-ember">HEAT</span>
            <span className="absolute right-6 bottom-3 text-[11px] font-mono font-bold text-brand-canvas/80">SWEET</span>
            <span className="absolute left-6 bottom-3 text-[11px] font-mono font-bold text-brand-umami">SMOKE</span>
            <span className="absolute left-1 top-1/3 text-[11px] font-mono font-bold text-brand-canvas/80">ACID</span>
          </div>
          ...
          <div className="mt-4 pt-4 border-t border-white/10 w-full flex items-center justify-between text-xs font-mono text-zinc-400">
            ...
            <span className="text-brand-ember font-bold">₹{activeSauce.price}</span>
          </div>
        </div>
        ...
        <div className="lg:col-span-6 space-y-5">
            ...
            {SAUCES.map((sauce) => (
              <button
                ...
                className={`p-3.5 rounded-2xl border text-left transition-all flex flex-col justify-between ${
                  activeSauce.name === sauce.name
                    ? 'bg-brand-dark text-brand-canvas border-brand-dark shadow-lg ring-2 ring-brand-ember'
              ...
              >
                ...
                <div className="flex items-center justify-between mt-2 font-mono text-[11px]">
                  <span className={activeSauce.name === sauce.name ? 'text-brand-glaze' : 'text-zinc-500'}>
                    ₹{sauce.price}
                  </span>
          ...
          <div className="pt-2">
            <button
              ...
              className="w-full py-4 rounded-2xl bg-gradient-to-r from-brand-ember to-brand-glaze text-white font-syne font-extrabold text-sm flex items-center justify-center gap-2 shadow-xl shadow-brand-ember/20 hover:brightness-105 transition"
            >
              {added ? (
                <>
